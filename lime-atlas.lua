...
function Atlas:setPosition(x, y)
	...
	if self.globe then
		local viewPoint = calculateViewpoint(self.globe, x, y)

		self.globe.x = round(viewPoint.x)
		self.globe.y = round(viewPoint.y)

		self.globe.x, self.globe.y = clampPosition(self.globe.x, self.globe.y, self.bounds)
