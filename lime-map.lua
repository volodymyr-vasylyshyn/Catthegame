...
function Map:updateLayerVisibility()
	...
	if _G.limeScreenCullingEnabled then
		...
		local updateVisuals = false
		
		if not self.previousCullPosition then
			self.previousCullPosition = {}
			self.previousCullPosition.x = self.world.x
			self.previousCullPosition.y = self.world.y
			
			updateVisuals = true
		end
		
		if ( self.previousCullPosition.x > ( self.world.x + self.header.tilewidth ) ) then
			updateVisuals = true
		elseif ( self.previousCullPosition.x < ( self.world.x - self.header.tilewidth ) ) then
			updateVisuals = true
		elseif ( self.previousCullPosition.y > ( self.world.y + self.header.tileheight ) ) then
			updateVisuals = true
		elseif ( self.previousCullPosition.y < ( self.world.y - self.header.tileheight ) ) then
			updateVisuals = true
		end
...
function Map:adjustClampingBoundsForScale()
	...
	if bounds then
		...
		bounds.width = bounds.width * self.world.xScale
		bounds.height = bounds.height * self.world.yScale
		
		if bounds.offset then
			bounds.offset.x = bounds.offset.x * self.world.xScale
		end
