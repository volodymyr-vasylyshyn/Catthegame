...
function isometricToWorldPosition(map, position)
...
function worldToIsometricPosition(map, position)
...
-- @returns The world position.
function gridToWorldPosition( map, position, offset )
...
function clampPosition(x, y, bounds)
	...
	local boundsWidth = _bounds.width 
	local boundsHeight = _bounds.height 
	
	if abs(_x) > boundsWidth + 50 then -- this is for scrolling to mid left side of the map
		_x = ( boundsWidth ) 
	elseif _x < 1 and abs(_x) > boundsWidth - display.contentWidth + 15 then  -- this is for scrolling to mid right side of map
		_x = - ( boundsWidth - display.contentWidth ) 
	elseif _x > _bounds.x then
		_x = _bounds.x
	end
	
	if _y < 0 and abs(_y) > abs(boundsHeight) - display.contentHeight then  -- this is for scrolling to mid bottom
		_y = - ( abs( boundsHeight ) - display.contentHeight)
	elseif _y > 0 and  _y > boundsHeight - display.contentHeight then -- this is for scrolling to mid top
		_y =  _bounds.y
